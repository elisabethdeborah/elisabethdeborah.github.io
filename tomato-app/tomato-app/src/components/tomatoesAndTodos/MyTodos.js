...

			{
				...
				<section className="no-tomatoes">
					...
					<h3 onClick={() => setViewAddTodoForm(!viewAddTodoForm)} className="time-tomato-header">Would you like to add one?</h3>
					{
						viewAddTodoForm ? 
						<AddTodoForm 
							todoName={todoName} viewAddTodoForm={viewAddTodoForm} setViewAddTodoForm={setViewAddTodoForm} saveTomatoObj={saveTomatoObj} objType="todo" setTodoName={setTodoName} 
						/>
						:null
					}
				</section>
						...
						<article key={todo.id} className={ todo.checked ? "tomato-article  checked-background": "tomato-article" } >
							<section className="todo-svg-container" onClick={ editMatch ?() => checked(todo):null}  >
					...
					
								{
									todo.checked ? 
									<FontAwesomeIcon 
										icon={faCheckCircle} onClick={() => checked(todo)} className="logo" style={{zIndex: '1', fill: '#555858'}} 
									/>
							...
							<TomatoTodoInfo 
								editMatch={editMatch} tomato={todo} setNewName={setNewName} setNewHours={setNewHours} setNewMinutes={setNewMinutes} setNewSeconds={setNewSeconds} 
							/>
