...
const SaveForm = ({ setTomatoHours, setTomatoMinutes, setTomatoSeconds, tomatoHours, tomatoMinutes,tomatoSeconds, setTomatoName, saveTomatoObj, stopwatchTime, setStopwatchTime, nameIsValid, handleCloseCountdown}) => {
	...
	return (
		<form className="save-form" method="POST" >
			...
			<input 
				className="tomato-name-input" type="text" placeholder="tomato name" 
				onChange={(e) => setTomatoName(e.target.value)} required 
				style={ !nameIsValid ? {border: '2px solid red'}: { border: '2px solid green'} } 
			/>
			<SaveNumberInput 
				stopwatchTime={stopwatchTime} setTomatoHours={setTomatoHours} setTomatoMinutes={setTomatoMinutes} setTomatoSeconds={setTomatoSeconds} 
			/>
			<section className="countdown-settings">
				<button type="button" value="save tomato" disabled={!nameIsValid || !stopwatchTime>0} onClick={() => saveTomatoObj('tomato')}>
					Save tomato
				</button>
